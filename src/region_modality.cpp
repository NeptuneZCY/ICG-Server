...
bool RegionModality::CalculateCorrespondences(int iteration,
  ...
  // 遍历2562views_，找当前姿态最接近的视点，点乘最小的，取出一组轮廓采样点
  ...
  // std::cout << "data_points.size()4: " << data_points.size() << std::endl;
  // std::cout << "orientation: " << view->orientation.matrix() << std::endl;
  ...
  for (int j = 0; j < 2; ++j) {
    ...
    // Iterate over n_lines 遍历200条对应线
...
bool RegionModality::CalculateGradientAndHessian(int iteration,
  ...
  for (auto &data_line : data_lines_) {
    ...
    // Calculate first derivative of loglikelihood with respect to delta_cs 计算loglikelihood相对于delta_cs的一阶导数
    ...
    if (opt_iteration < n_global_iterations_) {
      ...
      // std::cout<<"Body " << body_ptr_->name()<<std::endl;
      // std::cout<<"using learning_rate_: "<<learning_rate_<<std::endl;
    }

    // Calculate first order derivative of delta_cs with respect to theta 计算delta_cs相对于theta的一阶导数
    ...
    // total_delta_cs += weight * pow(fabs(delta_cs), alpha);
    // total_weight += weight;
  ...
  // std::cout << "err: " << total_delta_cs / total_weight << std::endl;
...
float RegionModality::CalculateError(float alpha=0.125f) {
  if (!IsSetup()) return false;
  ...
  for (auto &data_line : data_lines_) {
    ...
    total_error += weight * pow(fabs(delta_cs), alpha);
    total_weight += weight;
  }
  // std::cout << "err / : " << total_error / total_weight << std::endl;
