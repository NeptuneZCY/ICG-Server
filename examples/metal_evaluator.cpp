...
MetalEvaluator::MetalEvaluator(const std::string &name,
...
void MetalEvaluator::set_detector_folder(const std::string &detector_folder) {
  detector_folder_ = detector_folder;
}

void MetalEvaluator::set_evaluate_refinement(bool evaluate_refinement) {
  evaluate_refinement_ = evaluate_refinement;
}

void MetalEvaluator::set_use_detector_initialization(
    bool use_detector_initialization) {
  use_detector_initialization_ = use_detector_initialization;
}

void MetalEvaluator::set_use_matlab_gt_poses(bool use_matlab_gt_poses) {
  use_matlab_gt_poses_ = use_matlab_gt_poses;
}

void MetalEvaluator::set_run_sequentially(bool run_sequentially) {
  run_sequentially_ = run_sequentially;
}

void MetalEvaluator::set_use_random_seed(bool use_random_seed) {
  use_random_seed_ = use_random_seed;
}

void MetalEvaluator::set_n_vertices_evaluation(int n_vertices_evaluation) {
  n_vertices_evaluation_ = n_vertices_evaluation;
}

void MetalEvaluator::set_visualize_tracking(bool visualize_tracking) {
  visualize_tracking_ = visualize_tracking;
}

void MetalEvaluator::set_visualize_frame_results(bool visualize_frame_results) {
  visualize_frame_results_ = visualize_frame_results;
}

void MetalEvaluator::StartSavingImages(
    const std::filesystem::path &save_directory) {
  save_images_ = true;
  save_directory_ = save_directory;
}

void MetalEvaluator::StopSavingImages() { save_images_ = false; }

void MetalEvaluator::set_use_region_modality(bool use_region_modality) {
  use_region_modality_ = use_region_modality;
}

void MetalEvaluator::set_use_depth_modality(bool use_depth_modality) {
  use_depth_modality_ = use_depth_modality;
}

void MetalEvaluator::set_measure_occlusions_region(
    bool measure_occlusions_region) {
  measure_occlusions_region_ = measure_occlusions_region;
}

void MetalEvaluator::set_measure_occlusions_depth(bool measure_occlusions_depth) {
  measure_occlusions_depth_ = measure_occlusions_depth;
}

void MetalEvaluator::set_model_occlusions_region(bool model_occlusions_region) {
  model_occlusions_region_ = model_occlusions_region;
}

void MetalEvaluator::set_model_occlusions_depth(bool model_occlusions_depth) {
  model_occlusions_depth_ = model_occlusions_depth;
}

void MetalEvaluator::set_tracker_setter(
    const std::function<void(std::shared_ptr<icg::Tracker>)> &tracker_setter) {
  tracker_setter_ = tracker_setter;
}

void MetalEvaluator::set_refiner_setter(
    const std::function<void(std::shared_ptr<icg::Refiner>)> &refiner_setter) {
  refiner_setter_ = refiner_setter;
}

void MetalEvaluator::set_optimizer_setter(
    const std::function<void(std::shared_ptr<icg::Optimizer>)>
        &optimizer_setter) {
  optimizer_setter_ = optimizer_setter;
}

void MetalEvaluator::set_region_modality_setter(
...
void MetalEvaluator::set_region_model_setter(
...
void MetalEvaluator::set_depth_modality_setter(
...
void MetalEvaluator::set_depth_model_setter(
...
bool MetalEvaluator::Evaluate() {
  if (!set_up_) {
    std::cerr << "Set up evaluator " << name_ << " first" << std::endl;
    return false;
  }
  std::cout << "Evaluate" << std::endl;
  std::cout << "run_configurations_.size: " << run_configurations_.size() << std::endl;
...
void MetalEvaluator::SaveResults(std::filesystem::path path) const {
...
float MetalEvaluator::add_auc() const { return final_results_.at("all").add_auc; }

float MetalEvaluator::adds_auc() const {
  return final_results_.at("all").adds_auc;
}

float MetalEvaluator::execution_time() const {
  return final_results_.at("all").execution_times.complete_cycle;
}

std::map<std::string, MetalEvaluator::Result> MetalEvaluator::final_results()
    const {
  return final_results_;
}

bool MetalEvaluator::EvaluateRunConfiguration(
...
bool MetalEvaluator::SetUpTracker(
    const RunConfiguration &run_configuration,
    const std::shared_ptr<icg::RendererGeometry> &renderer_geometry_ptr,
  ...
  *tracker_ptr = std::make_shared<icg::Tracker>("tracker");
  ...
  // Init cameras
  std::filesystem::path camera_directory{dataset_directory_};
                                        
  // 
  auto color_camera_ptr{std::make_shared<icg::LoaderColorCamera>(
      "color_camera", dataset_directory_ / "color_camera.yaml")};
  color_camera_ptr->SetUp();
  auto depth_camera_ptr{std::make_shared<icg::LoaderDepthCamera>(
      "depth_camera", dataset_directory_ / "depth_camera.yaml")};
...
void MetalEvaluator::ResetBodies(
...
void MetalEvaluator::ExecuteMeasuredRefinementCycle(
...
void MetalEvaluator::ExecuteMeasuredTrackingCycle(
    const std::shared_ptr<icg::Tracker> &tracker_ptr, int load_index,
    int iteration, ExecutionTimes *execution_times) const {
  std::chrono::high_resolution_clock::time_point begin_time;
...
void MetalEvaluator::UpdateCameras(
...
MetalEvaluator::Result MetalEvaluator::CalculateAverageBodyResult(
...
MetalEvaluator::Result MetalEvaluator::CalculateAverageSequenceResult(
...
MetalEvaluator::Result MetalEvaluator::CalculateAverageBodyResult2(
    const std::vector<std::string> &body_names) const {
  size_t n_results = 0;
  std::vector<Result> result_sums;
  for (const auto &result : results_) {
...
MetalEvaluator::Result MetalEvaluator::CalculateAverageSequenceResult2(
    const std::vector<std::string> &sequence_names) const {
  size_t n_results = 0;
  std::vector<Result> result_sums;
  for (const auto &result : results_) {
...
MetalEvaluator::Result MetalEvaluator::CalculateAverageResult(
    const std::vector<Result> &results) {
  return DivideResult(SumResults(results), results.size());
}

MetalEvaluator::Result MetalEvaluator::SumResults(
    const std::vector<Result> &results) {
  Result sum;
...
MetalEvaluator::Result MetalEvaluator::DivideResult(const Result &result,
                                                size_t n) {
  Result divided;
...
void MetalEvaluator::CalculatePoseResults(
    const std::shared_ptr<icg::Tracker> tracker_ptr,
...
bool MetalEvaluator::LoadGTPoses(
...
bool MetalEvaluator::LoadMatlabGTPoses(
    const std::string &sequence_name, const std::string &body_name,
    std::vector<icg::Transform3fA> *gt_body2world_poses) const {
  // Open poses file
...
bool MetalEvaluator::LoadDetectorPoses(
    const std::string &sequence_name, const std::string &body_name,
...
void MetalEvaluator::VisualizeResult(const Result &result,
...
bool MetalEvaluator::CreateRunConfigurations() {
  run_configurations_.clear();

  for (const auto &body_name : evaluated_body_names_) {
    // std::cout << body_name << std::endl;
    RunConfiguration run_configuration;
    run_configuration.sequence_id = 666;
    run_configuration.sequence_name = body_name;
...
void MetalEvaluator::AssambleTrackedBodyNames() {
...
bool MetalEvaluator::LoadBodies() {
  bool success = true;
  body2body_ptr_map_.clear();
#pragma omp parallel for
  for (int i = 0; i < tracked_body_names_.size(); ++i) {
    auto &body_name{tracked_body_names_[i]};
    std::filesystem::path directory{dataset_directory_};
    auto body_ptr{
        std::make_shared<icg::Body>(body_name, directory / (body_name + ".obj"), 1.0f,
...
bool MetalEvaluator::GenerateModels() {
  std::filesystem::path directory{dataset_directory_};
...
bool MetalEvaluator::LoadKeyframes() {
  bool success = true;
...
#pragma omp parallel for
  for (int i = 0; i < sequence_ids_.size(); ++i) {
    ...
    // Open poses file
    std::filesystem::path path{dataset_directory_ / "ground_truth" /"keyframe.txt"};
    std::ifstream ifs{path.string(), std::ios::binary};
    if (!ifs.is_open() || ifs.fail()) {
      ifs.close();
...
void MetalEvaluator::LoadNFrames() {
...
void MetalEvaluator::LoadPoseBegin() {
...
void MetalEvaluator::GenderateReducedVertices() {
#pragma omp parallel for
  for (int i = 0; i < tracked_body_names_.size(); ++i) {
    auto &body_name{tracked_body_names_[i]};
...
void MetalEvaluator::GenerateKDTrees() {
#pragma omp parallel for
  for (int i = 0; i < tracked_body_names_.size(); ++i) {
    auto &body_name{tracked_body_names_[i]};
    const auto &vertices{body2body_ptr_map_[body_name]->vertices()};
...
bool MetalEvaluator::BodyExistsInSequence(const std::string &sequence_name,
...
bool MetalEvaluator::SequenceBodyNames(
...
int MetalEvaluator::NFramesInSequence(const std::string &sequence_name) const {
...
//保存结果
void MetalEvaluator::SaveMyResults(std::filesystem::path path) {
  path += "/";
  // path += std::to_string(learning_rate_);
  // path += std::to_string(id_);
  path += "inner_learning_rate:";
...
void MetalEvaluator::set_learning_rate(float learning_rate){
  learning_rate_ = learning_rate;
}

void MetalEvaluator::set_use_gray(bool use_gray){
  use_gray_ = use_gray;
}

void MetalEvaluator::set_use_kalman(bool use_kalman){
  use_kalman_ = use_kalman;
}

void MetalEvaluator::set_sample_num(int sample_num){
  sample_num_ = sample_num;
}

void MetalEvaluator::set_kalman_Q(double kalman_Q){
  kalman_Q_ = kalman_Q;
}

void MetalEvaluator::set_kalman_R(double kalman_R){
  kalman_R_ = kalman_R;
}

void MetalEvaluator::set_kalman_start_frame(int kalman_start_frame){
  kalman_start_frame_ = kalman_start_frame;
}

void MetalEvaluator::set_id(int id){
  id_ = id;
}

void MetalEvaluator::set_errT_num(int errT_num){
  errT_num_ = errT_num;
}

// 设置
bool MetalEvaluator::SetUp() {
  set_up_ = false;
  // std::cout << "CreateRunConfigurations" <<std::endl;
  // if (!CreateRunConfigurations()) return false;
  // std::cout << "AssambleTrackedBodyNames" <<std::endl;
  // AssambleTrackedBodyNames();
  // std::cout << "LoadBodies" <<std::endl;
  // if (!LoadBodies()) return false;
  // std::cout << "GenerateModels" <<std::endl;
  // if (!GenerateModels()) return false;
  // std::cout << "LoadKeyframes" <<std::endl;
  // if (!LoadKeyframes()) return false;
  // std::cout << "LoadNFrames" <<std::endl;
  // LoadNFrames();
  // std::cout << "LoadPoseBegin" <<std::endl;
  // LoadPoseBegin();
  // std::cout << "GenderateReducedVertices" <<std::endl;
  // GenderateReducedVertices();
  // std::cout << "GenerateKDTrees" <<std::endl;
  // GenerateKDTrees();
...
bool MetalEvaluator::EvaluateTracker(std::shared_ptr<icg::Tracker> &tracker_ptr){
  // Start tracking
  if (!tracker_ptr->SetUp()) return false;
  ...
  const auto &body_name{tracker_ptr->body_ptrs()[0]->name()};
  if (use_matlab_gt_poses_) {
    // std::cout << "LoadMatlabGTPoses running......................................................." << std::endl;
    if (!LoadMatlabGTPoses(body_name, body_name,
                            &gt_body2world_poses[body_name]))
      return false;
  } else {
    // std::cout << "LoadGTPoses running......................................................." << std::endl;
    if (!LoadGTPoses(body_name, body_name,
                      &gt_body2world_poses[body_name]))
      return false;
  }
  // if (evaluate_refinement_) {
  ...
  int sequence_id = 0;
  for(int i=0; i<evaluated_body_names_.size(); i++){
    if(evaluated_body_names_[i] == body_name) {
      sequence_id = i;
      break;
    }
  }
  ...
  //buffer_size
  std::vector<int> n_keyframes_vec = {891, 831, 1035, 735};

  int n_keyframes = n_keyframes_vec[sequence_id];

  //Init results
  if(sequence_results.size() < evaluated_body_names_.size()) sequence_results.resize(evaluated_body_names_.size());
  
  auto &sequence_result = sequence_results[sequence_id];
  // std::cout << "evaluated_body_names.size(): " << evaluated_body_names.size() << std::endl;

  (sequence_result).body_name = body_name;
  (sequence_result).frame_results.clear();

  for (int iteration = 0; iteration < n_keyframes; ++iteration) {
    Result general_results;
    general_results.frame_index = iteration;

    // std::cout << "iteration: " << iteration << std::endl;
    auto begin{std::chrono::high_resolution_clock::now()};

    if (!tracker_ptr->UpdateCameras(iteration == 0)) return false;

    if (iteration == 0) {
      // if (!tracker_ptr->ExecuteDetectionCycle(iteration)) return false;
      // auto &body_ptr{tracker_ptr->body_ptrs()[0]};
      tracker_ptr->body_ptrs()[0]->set_body2world_pose(gt_body2world_poses[body_name][0]);
    }

    if (iteration == 0) {
      if (!tracker_ptr->StartModalities(iteration)) return false;
    }

    if (!tracker_ptr->ExecuteTrackingCycle(iteration)) return false;


    // 全局搜索模块
    float errMin = tracker_ptr->CalculateError(alpha_);
    // std::cout << "errMin: " << errMin << std::endl;

    // bool global_search = true;
    // 第0,1帧不判断前n帧，因为第1帧优化结束后才可以记录theta
    if(global_search_ && iteration > 1){
      float errT = tracker_ptr->GetMedianOfLastNError(errT_num_);
      // std::cout << "errT: " << errT << std::endl;
      ...
      if(errMin > errT){
        ...
        if(scan_grid){
          ...
          for(auto& curSeed: seed_vec){
            ...
            icg::Modality* m_ptr = modality_ptr.get();
            icg::RegionModality* region_modality_ptr = dynamic_cast<icg::RegionModality*>(m_ptr);
            region_modality_ptr->set_learning_rate(inner_learning_rate_);
    ...
    // const auto &body_name{body_ptrs()[0]->name()};
    Result result{general_results};
    Result stable_result{general_results}; // 抖动adds计算结果
    CalculatePoseResults(tracker_ptr, body_name,
                          gt_body2world_poses[body_name][iteration], &result, &stable_result);
    // std::cout << "gt_body2world_poses: " << gt_body2world_poses[body_name][iteration].matrix() << std::endl;

    sequence_result.frame_results.push_back(std::move(result));
    sequence_result.stable_results.push_back(std::move(stable_result));
...
bool MetalEvaluator::CalculateAllResult(){
