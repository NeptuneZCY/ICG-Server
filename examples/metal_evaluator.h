...
#ifndef ICG_EXAMPLES_METAL_EVALUATOR_H_
#define ICG_EXAMPLES_METAL_EVALUATOR_H_
...
class MetalEvaluator {
 ...
 private:
  ...
  struct SequenceResult {
    ...
    std::vector<Result> stable_results{};
    Result average_result{};
    Result average_stable_result{};
  };

 public:
  // Constructors and setup method
  MetalEvaluator(const std::string &name,
...
public:
  ...
  Result CalculateAverageBodyResult2(
      const std::vector<std::string> &body_names) const;
  Result CalculateAverageSequenceResult2(
      const std::vector<std::string> &sequence_names) const;
...
private:
    ...
    double kalman_Q_;
    double kalman_R_;
    int kalman_start_frame_;
	std::string time_;
    std::map<std::string, std::vector<std::pair<float, float>>> body_learning_rate_control_map{};// 记录物体每帧的预测帧变化程度（adds）与学习率
    std::map<std::string, std::vector<float>> map_temp{};

public:
	void SaveMyResults(std::filesystem::path);
    void set_id(int);
	void set_learning_rate(float);
    void SaveLearningRateByBody(std::filesystem::path);
	// void Mkdir(std::filesystem::path path);
    void set_use_gray(bool);
    void set_use_kalman(bool);
    void set_sample_num(int);
    void set_kalman_Q(double);
    void set_kalman_R(double);
    void set_kalman_start_frame(int);

private:
    Kalman* kalman = nullptr;

public:
    void set_errT_num(int);
    int errT_num_;
    float alpha_;
    bool global_search_;
    float inner_learning_rate_;

    bool EvaluateTracker(std::shared_ptr<icg::Tracker> &tracker_ptr);
    // std::map<std::string, SequenceResult> sequence_results_map;
    std::vector<SequenceResult> sequence_results;
...
#endif  // ICG_EXAMPLES_METAL_EVALUATOR_H_
