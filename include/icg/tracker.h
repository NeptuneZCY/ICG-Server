...
class Tracker {
 public:
  ...
  bool RunTrackerProcessOneByOne(bool execute_detection = false,
                         bool start_tracking = false);
  bool EvaluateRunTrackerProcess(bool execute_detection = false,
                         bool start_tracking = false);
...
public:
  // 记录所有物体的上帧位姿
  std::map<std::string, icg::Transform3fA> body2world_pose_map_{};
  // 记录所有物体的每帧位姿
  std::map<std::string, std::vector<icg::Transform3fA>> body2world_pose_map_vector_{};
  // 记录所有物体的过去n帧位姿
  std::map<std::string, std::deque<icg::Transform3fA>> body2world_pose_map_deque_{};
  // 记录所有物体的过去n帧匹配错误
  std::map<std::string, std::deque<float>> error_map_deque_{};
  // 记录所有物体的过去n帧搜索角度范围
  std::map<std::string, std::deque<float>> theta_map_deque_{};
  // // 记录每个序列所有物体的过去n帧信息
  // std::map<std::string, std::map<std::string, std::deque<icg::FrameInfo>>> sequence_name_deque_frameinfo{};
  // std::map<std::string, float*> error_sum_map_{};
  // 记录位姿
  void RecordLastPose();
  // 记录匹配错误
  void RecordError(float);
  // 记录搜索角度范围
  void RecordTheta(float);

  // 标注位姿
  bool PoseAnnotation_ = false;
  std::filesystem::path poseSavePath_;
  void startPoseAnnotationAndSave(const std::filesystem::path &poseSavePath);
  void savePose();
  // void savePose(const std::filesystem::path &poseSavePath, std::shared_ptr<Body> &body);

  // 计算匹配误差
  float CalculateError(float alpha);
  // 计算角度搜索范围
  float CalculateTheta();

  // // 保存信息
  // bool saveFrame();
  // bool createFrameInfo();

  float GetMedianOfLastN(const std::deque<float>& recorded_deque, int nMax);
  float GetMedianOfLastNError(int n);
  float GetMedianOfLastNTheta(int n);

  float GetRDiff(const cv::Matx33f& R1, const cv::Matx33f& R2);
