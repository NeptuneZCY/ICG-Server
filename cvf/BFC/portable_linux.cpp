...
_BFC_API bool copyFile(const string_t &src, const string_t &tar, bool overWrite)
{
    ...
    try
    {
        copy_file(path(src),path(tar),overWrite? copy_option::overwrite_if_exists:copy_option::none);
    }
    catch(...)
    {
        ok=false;
...
{
	boost::filesystem::current_path(path(dir));
}
...
{
	return boost::filesystem::exists(filesystem::path(path));
}
_BFC_API bool  isDirectory(const string_t &path)
{
	return boost::filesystem::is_directory(filesystem::path(path));
}
_BFC_API string_t getFullPath(const string_t &path)
{
	return boost::filesystem::system_complete(filesystem::path(path)).string();
}
...
_BFC_API void   listFiles(const string_t &path, std::vector<string_t> &files, bool recursive, bool includeHidden)
{
	...
	if (!recursive)
	{
		boost::filesystem::directory_iterator itr(path), end;
	...
	{
		boost::filesystem::recursive_directory_iterator itr(path), end;
		for (;itr!=end;++itr)
		{
			if (is_regular_file(itr->status()) && (includeHidden || !isHidden(*itr)))
...
{
	boost::filesystem::remove_all(filesystem::path(path));
}
